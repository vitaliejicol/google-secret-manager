...
variable "secret" {
  type        = string
  default     = "Change_the_Secret1s" ##after testing remove this line, secret will be passed over by the upstream module
  sensitive   = true
  description = <<EOD
...
variable "accessors" {
  type    = list(string)
  default = [
    "serviceAccount:my-service-account@arctic-welder-274614.iam.gserviceaccount.com"
    ]
  validation {
  ...
  type = map(object({
    ...
    default = {
      "default" = {
        ...
        next_rotation_time = "2014-10-02T15:01:23.045123456Z"
